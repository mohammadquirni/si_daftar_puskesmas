...
class Jadwal_Periksa {
	...
	function getNewAntrian() {
		$query = "SELECT MAX(nomor_antrian) AS code FROM {$this->table_jadwal_periksa} WHERE tgl_periksa = '{$this->today}'";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	function readAll() {
		$query = "SELECT A.id_jadwal_periksa, B.nama AS nama_dokter, C.nama_poli, A.tgl_periksa, A.gejala_penyakit, A.berat_badan FROM {$this->table_jadwal_periksa} A LEFT JOIN {$this->table_dokter} B ON A.id_pasien=B.id_pasien LEFT JOIN {$this->table_poli} C ON A.id_poli= C.id_poli ORDER BY id_jadwal_periksa ASC";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
